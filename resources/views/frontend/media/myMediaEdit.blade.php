...
@if(!$get_media->isEmpty())
    @if($get_media[0]->all_recipient != null) {
        @foreach($get_media[0]->all_recipient as $recipent) {
        ...
        <form method="POST" action="{{ route('user.media.update-media') }}" enctype="multipart/form-data" onsubmit="return validateForm(this)">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="row">
                        <div class="col-md-12">
                            @if($get_media[0]->type == 'photo')
                                <picture id="ban_image" class="tv_image"><img src="{{ asset( $file_path.$get_media[0]->file_name )}}" type="image" height="500" width="720" /></picture>
                            @elseif($get_media[0]->type == 'video')
                                <video id="ban_video" class="tv_video" controls><source src="{{ asset( $file_path.$get_media[0]->file_name )}}" type="video/mp4" /></video>
                            @elseif($get_media[0]->type == 'audio')
                                <audio id="ban_audio" class="tv_audio mt-4" controls><source src="{{ asset( $file_path.$get_media[0]->file_name )}}" type="audio/mp3" /></audio>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        <div class="mb-3 w-100">
                            ...
                            <input type="text" id="title" name="title" value="{{ $get_media[0]->title }}" class="form-control capture-form" placeholder="Title Here" required>
                            <div class="col-12" id="show_title_msg"></div>
                        </div>
                        <div class="mb-3  w-100">
                            <label for="description" class="form-label text-white">Description</label>
                            <textarea class="form-control capture-form" id="description" name="description" placeholder="Description Here" required>{{ $get_media[0]->description }}</textarea>
